<?php // $Id: addendofbranch.php 641 2011-07-21 18:55:58Z griffisd $
/**
 * Action for adding an end of branch page
 *
 * @version $Id: addendofbranch.php 641 2011-07-21 18:55:58Z griffisd $
 ...
 **/
    ...
    $btpageid = $pageid;
    if (!$btpage = get_record("languagelesson_pages", "id", $btpageid)) {
        error("Add end of branch: btpage record not found");
    }
    while (($btpage->qtype != LL_BRANCHTABLE) AND ($btpage->prevpageid > 0)) {
    ...
    if ($btpage->qtype == LL_BRANCHTABLE) {
        ...
        languagelesson_set_message(get_string('addedanendofbranch', 'languagelesson'), 'notifysuccess');
    } else {
        languagelesson_set_message(get_string('nobranchtablefound', 'languagelesson'));
    }
