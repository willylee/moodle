...
class LanguageLessonPageViewer {
    ...
    public function view() {
        ...
        if ($pageid == LL_UNSEENBRANCHPAGE) {
            $pageid = languagelesson_unseen_question_jump($this->lesson->id, $USER->id, $pageid);
        }
        
        // if the pageid is empty, then we are coming from the course page
        if (empty($pageid)) {
            // check if there is any lesson content
            $this->verify_has_pages();
    
            // log that this lesson got started   
            add_to_log($course->id, 'lesson', 'start', 'view.php?id='. $cm->id, $this->lesson->id, $cm->id);
            
            // check to see if this user has completed the lesson before
            $hascompleted = false;
            // if no pageid given see if the lesson has been started
            if (!has_capability('mod/languagelesson:manage', $this->context)
                    && languagelesson_is_lesson_complete($this->lesson->id, $USER->id)) {
                $hascompleted = true;
            }
            ...
            if ($this->lesson->type != LL_TYPE_PRACTICE
                    ...
                    && !has_capability('mod/languagelesson:manage', $this->context)) {
                $this->print_old_grade_page(); // will terminate script
            }

            // otherwise, log that they started the LL for the first time
            $this->startLL();
        }
    
        else if ($pageid != LL_EOL) {
            ...
            $this->print_page_head($lastattemptwarning, $nomoreattempts);

            $this->print_prompt();
        
            list($oldattempt, $img) = $this->handle_old_attempt();
            ...
            if ($answers = get_records("languagelesson_answers", "pageid", $page->id, "id")) {
                ...
                switch ($page->qtype) {
                    ...
                    case LL_SHORTANSWER :
                    //case LL_NUMERICAL :
                        $this->print_answers_shortanswer();
                        break;
                    case LL_CLOZE :
                        $this->print_answers_cloze($oldattempt, $img);
                        break;
                    case LL_TRUEFALSE :
                        $this->print_answers_truefalse($answers, $oldattempt, $img);
                        break;
                    case LL_MULTICHOICE :
                        $this->print_answers_multichoice($answers, $oldattempt, $img);
                        break;
                    case LL_MATCHING :
                        $this->print_answers_matching($answers, $oldattempt, $img);
                        break;
     ...
     *   2) Any dependencies
     **/
    private function check_ll_availability() {
    ...
    private function checkTimeConstraints() {
        if (($this->lesson->available != 0 and time() < $this->lesson->available) or
            ...
            languagelesson_print_header($cm, $course, $this->lesson);
     ...
     **/
    private function verify_has_pages() {
        if (!get_field('languagelesson_pages', 'id', 'lessonid', $this->lesson->id, 'prevpageid', 0)) {
            ...
            if (!has_capability('mod/languagelesson:manage', $this->context)) {
                languagelesson_set_message(get_string('lessonnotready', 'languagelesson', $course->teacher));
    ...
    private function print_old_grade_page() {
        $grade = get_record('languagelesson_grades', 'lessonid', $this->lesson->id, 'userid', $USER->id);
        
        languagelesson_print_header($cm, $course, $this->lesson, 'view');
        print_simple_box_start('center');
        echo "<div style=\"text-align:center;\">";
    ...
    private function startLL() {
        ...
        if(!isset($USER->startlesson[$this->lesson->id]) && !has_capability('mod/languagelesson:manage', $this->context)) {
    ...
    private function handle_structural_redirects() {
        ...
        // handle the end of a branch
            ...
            redirect("$CFG->wwwroot/mod/languagelesson/view.php?id=$cm->id&amp;pageid=$page->nextpageid");
        }
    ...
    private function set_attempt_warning_flags() {
        ...
        if ($this->lesson->maxattempts > 0 &&  // if maxattempts=0, attempts are unlimited
                !has_capability('mod/languagelesson:manage', $this->context)) { // show this only to students
    ...
    private function print_page_head() {
        // basic page setup
        $PAGE = page_create_instance($this->lesson->id);
        ...
        if ($this->lesson->showongoingscore and !empty($pageid)) {
        ...
        <table id="layout-table" cellpadding="0" cellspacing="0">
            <tr>
                <!-- First Column -->
                <?php if (languagelesson_blocks_have_content($this->lesson, $pageblocks, BLOCK_POS_LEFT)) { ?>
                <td id="left-column" style="width: <?php echo $leftcolumnwidth; ?>px;">
                    <?php
                        languagelesson_print_menu_block($cm->id, $this->lesson);
     ...
     **/
    private function handle_old_attempt() {
        ...
        else if (has_capability('mod/languagelesson:manage', $this->context) && $this->lesson->showoldanswer) {
     ...
     */
    private function print_answers_shortanswer($answers, $oldattempt, $img) {
        print_simple_box_start("center");
     ...
     */
    private function print_answers_cloze($answers, $oldattempt, $img) {
        print_simple_box_start("center");
     ...
     */
    private function print_answers_truefalse($answers, $oldattempt, $img) {
        print_simple_box_start("center");
        echo '<table id="answertable">';
     ...
     */
    private function print_answers_multichoice($answers, $oldattempt, $img) {
        print_simple_box_start("center");
        echo '<table id="answertable">';
     ...
     */
    private function print_answers_matching($answers, $oldattempt, $img) {
        print_simple_box_start("center");
        echo '<table id="answertable">';
    ...
    private function print_branchtable_jumps() {
        ...
        $branches = get_records('languagelesson_branches', 'parentid', $pageid, 'ordering');
    ...
    private function print_answer_essay($oldattempt) {
        ...
        if ($attempt = languagelesson_get_most_recent_attempt_on($page->id, $USER->id)) {
            ...
            languagelesson_print_submission_feedback_area($manattempt, $page->qtype, $this->showoldattempt);
            
            // if the lesson is set to show the old attempt, plug their old submission into the WYSIWYG
            error_log("showoldattempt is $this->showoldattempt");
    ...
    //////////////////////////////////////////////////////
        ...
        /// display feedback and print the submit/continue button
            ...
            /// feedback
                ...
                if ($showfeedback) {
                    ...
                    if ($page->qtype != LL_AUDIO
                        ...
                        if ((!has_capability('mod/languagelesson:manage', $this->context) && $attempt->correct)
                                || (has_capability('mod/languagelesson:manage', $this->context) && $answer->score > 0)) {
            ...
            /// nav buttons
                ...
                if ($this->showoldattempt || $nomoreattempts || $reviewing) {
                    echo '<td>';
    ...
    //////////////////////////////////////////////////////
        ...
        languagelesson_print_progress_bar($this->lesson, $course);
        ...
        ?>
                ...
                <?php if (languagelesson_blocks_have_content($this->lesson, $pageblocks, BLOCK_POS_RIGHT)) { ?>
                <td id="right-column" style="width: <?php echo $rightcolumnwidth; ?>px;">
                    <?php
                        languagelesson_print_clock_block($cm->id, $this->lesson, $timer);
                        languagelesson_print_mediafile_block($cm->id, $this->lesson);
...
///////////////////////////////////////////////////////////
    ...
    /// update the user's timer record
        if (!has_capability('mod/languagelesson:manage', $this->context)) { // if the user is a student
            unset($USER->startlesson[$this->lesson->id]);
            if (!$timer = get_records_select('languagelesson_timer', "lessonid = $this->lesson->id AND userid = $USER->id", 'starttime')) {
                //error('Error: could not find records'); ///TODO: find what causes this error, and get rid of the causes!  Shouldn't
                    //just ignore the error...
                languagelesson_insert_bs_timer($this->lesson->id, $USER->id);
    ...
    /// log that they got to the end of the lesson
        add_to_log($course->id, "lesson", "end", "view.php?id=$cm->id", "$this->lesson->id", $cm->id);
