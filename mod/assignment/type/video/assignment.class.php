 ...
 *
 */
class assignment_video extends assignment_base {
    ...
    function assignment_video($cmid='staticonly', $assignment=NULL, $cm=NULL, $course=NULL) {
    ...
    function view() {
		...
		// if there is a submission, print data in this order (if present):
		// File w/ embedded player
		// Grade/text feedback
		// Audio feedback player(s)
        if ($submission = $this->get_submission()) {
			...
			// view_feedback will print nothing if there is no feedback to show
			$this->view_feedback();
    ...
    function view_feedback($submission=NULL) {
		...
		if ($submission->timemarked) {
			...
			if (!$grade->hidden && $grade->grade !== false && $grade->usermodified != $USER->id) {
				...
				if (count($contents) > 0) {
					...
					echo '<div class="revletInstructions">'.get_string('feedbackplayerinstructionssimple', 'assignment').'</div>';
					...
					// pull the path to the submitted feedback file
					$fbdir = $this->feedback_area_name($submission->userid);
					$thisfile = array_pop($contents);
					$furl = "$CFG->wwwroot/file.php/$fbdir/$thisfile";
					...
					echo "\t\tstudentfile=\"$furl\"\n"; //path to the TEACHER RESPONSE file to be downloaded
					echo "\t\tfeedbackfnames=\"\"\n";				
		...
		if (count($teacherIDs) > 0) {
			...
			$flag = false;
			foreach ($teacherIDs as $teacherid) {
				echo "<div id='{$basename}{$teacherid}' style='display:none'>";
				//echo "<p>".$fbarr['text']."</p>";
				...
				echo '<p class="revletInstructions">'.get_string('feedbackplayerinstructionssimple', 'assignment').'</p>';

				// get the teacher's response file
				$thisfeedbackdir = $feedbackdir . '/' . $teacherid;
				$files = get_directory_list($thisfeedbackdir);
				if (count($files) != 1) { error('This teacher recorded too many (or too few) feedback files! Tell them to consolidate
						their comments.'); }
				$thisfile = array_pop($files);
				$furl = "$thisfeedbackdir/$thisfile";
	...
	function print_recorder($stuid) {
		global $CFG;

		echo '<p>'.get_string('simplefeedbackinstructions', 'assignment').'</p>';

        include("feedback/simplerecorder/revA.php");
        ...
        echo "\t\tuploadparams=\"id=" . $this->cm->id . "&sesskey=" . sesskey() . "&feedback=1&stuid=$stuid\"\n"; 

		// print out the javascript to be run on submission
		echo "\t\tsubmitscript=\"document.forms['submitform'].elements['submit'].click();\"\n";

		// pull in closing </embed> and </object> tags, and javascript to handle prompting revWeb plugin
        readfile($CFG->wwwroot . "/mod/assignment/feedback/simplerecorder/revB.html");
	}
    ...
    function view_upload_form() {
		...
		// pull in opening <object> and <embed> tags
		include("type/video/revA.php");

		// print out custom embed params
        echo "\t\tMoodleSession=\"". $_COOKIE['MoodleSession'] . "\"\n" ;  
        echo "\t\tMoodleSessionTest=\"" . $_COOKIE['MoodleSessionTest'] . "\"\n";
        echo "\t\tMOODLEID=\"" . $_COOKIE['MOODLEID_'] . "\"\n"; 
        echo "\t\tsesskey=\"" . sesskey() . "\"\n"; 
		...
		// pull in closing </embed> and </object> tags, and jscript to handle prompting for RevWeb plugin
        readfile($CFG->wwwroot . "/mod/assignment/type/video/revB.html");
        echo '</div>';
    }
    ...
    function upload() {
		...
		error_log("video-type assignment: upload function called");
		...
		if (count($_FILES) > 0) {
			error_log("video-type assignment: temp files array includes at least one file");
		} else {
			error_log("video-type assignment: ERROR: temp files array is empty");
		}
        ...
        if ($this->isopen() && (!$filecount || $this->assignment->resubmit || !$submission->timemarked)) {
            ...
            if ($um->process_file_uploads($dir) and confirm_sesskey()) {
				...
				error_log("video-type assignment: upload succeeded");
			...
			error_log("video-type-assignment: ERROR: upload failed");
        }
    ...
    function print_user_files($userid=0, $return=false, $autoplay=false) {
        global $CFG, $USER;

		echo '<script type="text/javascript">window.resizeTo(985, 700);</script>';
        ...
        if ($basedir = $this->file_area($userid)) {
            if ($files = get_directory_list($basedir, '', false)) {
                ...
                foreach ($files as $key => $file) {
                    ...
                    $embedded_file = format_text('<a href="'.$ffurl.'" >'.$file.'</a><br />', "0", $formatoptions);
                    $output .= '<img src="'.$CFG->pixpath.'/f/'.$icon.'" class="icon" alt="'.$icon.'" />'. $embedded_file ;
				...
				*/
				...
				if (count($files) != 1) { error('Wrong number of files found for a video-type assignment.'); }
				...
				$embedder .= 'width="320" height="256">';
				$embedder .= 	'<param name="pluginspage" value="http://www.apple.com/quicktime/download/" />
						 <param name="src" value="'.$ffurl.'" />
				...
				$embedder .= '<object data="'.$ffurl.'" type="video/quicktime"';
				$embedder .= ' width="320" height="256">';
						
				$embedder .= '<param name="src" value="'.$ffurl.'" />
